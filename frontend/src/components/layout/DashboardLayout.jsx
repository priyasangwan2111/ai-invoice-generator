...
const DashboardLayout = ({children,activeMenu}) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 ">
      <div classNmae={`flex insert-y-0 left-0 z-50 transition-transform duration-300 transform ${
        isMobile?
           sidebarOpen?
           "translate-x-0":
           "-translate-x-full"
        ...
        <div className="flex items-center h-16 border-b border-gray-200 px-6">
          <Link className="flex items-center space
          " to ='/dashboard'>
        ...
        <nav className="p-4 space-y-2">
            {NAVIGATION_MENU.map((item)=>(
              <NavigationItem key={item.id} item={item} isActive={activeNavItem===item.id} onClick={handleNavigation}
        ...
        <div className="absolute bottom-4 left-4 right-4" >
          <button
           className="w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-lg text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 hover"
           onClick={logout}
           >
      ...
      {isMobile &&  sidebarOpen &&(
        <div className="fixed inset-0 bg-black/10 bg-opacity-25 z-40 backdrop-blur-sm"
          ...
          <div className="flex items-center space-x-4">
            {
              isMobile &&(
                ...
                className="p-2 rounded-xl hover:bg-gray-100 transition-color duration-200">
                  {sidebarOpen ?(
